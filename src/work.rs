...
/// the runtime catches it and reports via the task's [`WorkListener::on_error`](crate::WorkListener::on_error); other tasks
/// on the same execution thread are not affected.
///
/// # Performance note
///
/// This trait uses [`async_trait`](macro@async_trait) under the hood, which means **each call to
