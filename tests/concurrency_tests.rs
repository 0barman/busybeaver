...
async fn test_high_frequency_execution() -> BeaverResult<()> {
    ...
    // 1ms sleep actually takes ~15ms). We only assert the periodic loop fired
    // repeatedly several times within the window; the exact rate is
    // environment-dependent.
    assert!(
        count >= 3,
        "High frequency should execute repeatedly, got {}",
        count
    );
...
async fn test_long_resident_survives_cancel() -> BeaverResult<()> {
    ...
    beaver
        .enqueue_on_new_thread(long_task, "long-dam", 256, true)
        .await?;
    beaver
        .enqueue_on_new_thread(short_task, "short-dam", 256, false)
        .await?;
...
async fn test_multiple_long_resident_dams() -> BeaverResult<()> {
    ...
    for (i, counter) in counters.iter().enumerate() {
        ...
        beaver
            .enqueue_on_new_thread(task, format!("long-dam-{}", i), 256, true)
            .await?;
    }
...
async fn test_mixed_task_types() -> BeaverResult<()> {
    ...
    beaver
        .enqueue_on_new_thread(periodic_task, "periodic-dam", 256, false)
        .await?;
    beaver
        .enqueue_on_new_thread(fixed_task, "fixed-dam", 256, false)
        .await?;
    beaver
        .enqueue_on_new_thread(interval_task, "interval-dam", 256, false)
        .await?;
...
async fn test_reuse_dam_name_after_release() -> BeaverResult<()> {
    ...
    beaver
        .enqueue_on_new_thread(task1, "reusable-dam", 256, false)
        .await?;
    tokio::time::sleep(Duration::from_millis(50)).await;

    // Release
    beaver
        .release_thread_resource_by_name("reusable-dam")
        .await?;
    ...
    beaver
        .enqueue_on_new_thread(task2, "reusable-dam", 256, false)
        .await?;
    tokio::time::sleep(Duration::from_millis(50)).await;
...
async fn test_empty_dam_name() -> BeaverResult<()> {
    ...
    beaver.enqueue_on_new_thread(task, "", 256, false).await?;
